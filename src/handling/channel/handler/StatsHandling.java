...
public class StatsHandling {
    ...
    public static final void AutoAssignAP(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        //System.out.println("AutoAssignAP Handling received!");
        chr.updateTick(slea.readInt());
        //slea.skip(4);
        ...
        try{
            if(count>1)
            {
                SecondaryStat = GameConstants.GMS ? (int) slea.readLong() : slea.readInt();
                amount2 = slea.readInt();
                System.out.println("Auto Assigned AP for Xenon character!");
            }
        }
        catch(Exception e1)
        {
            System.out.println("Auto Assigned AP for non-Xenon character!");
        }
        
        //System.out.println("AutoAssignAP Preparing: ["+PrimaryStat+": "+amount+"], ["+SecondaryStat+": "+amount2+"]");
    ...
    /*    if (chr.getRemainingAp() == amount + amount2) {
            ...
            //System.out.println("AutoAssignAP Checking: ["+chr.getRemainingAp()+"/"+ amount+amount2 +"].");
           if (chr.getRemainingAp() >= amount + amount2) {
            //System.out.println("Setting Primary Stat to "+PrimaryStat+" value "+amount);
            switch (PrimaryStat) {
            ...
            if(count>1)
            {
            System.out.println("Setting Secondary Stat to "+SecondaryStat+" value "+amount2);
